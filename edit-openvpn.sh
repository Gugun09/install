...
if [ $? -eq 0 ]; then
	...
	sed -i $Port_Change /etc/openvpn/client.ovpn
	sed -i $Port_Change /etc/openvpn/openvpnssl.ovpn
	...
	sed -i $Port_Change /etc/stunnel/ssl.conf
	sed -i $Port_Change /root/stunnel4port
	stunnel4port="$(netstat -nlpt | grep -i stunnel | grep -i 0.0.0.0 | awk '{print $4}' | cut -d: -f2)"
	cat > /root/stunnel4port <<-END
	$stunnel4port
	END
